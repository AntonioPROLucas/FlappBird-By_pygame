...
IMG_DE_FUNDO = pygame.transform.scale2x(pygame.image.load(os.path.join("imgs", "bg.png")))
IMG_PASSARO = [
...
pygame.font.init()
FONTE_PTS = pygame.font.SysFont('arial', 50)
...
class Passaro:
    ...
    def mover (self):
        ...
        self.y += deslocamento
        #angulo do passaro
        if deslocamento < 0 or self.y < self.altura + 50: # Começa a rotacionar para cima quando está subindo
            if self.angulo < self.ROTACAO_MAXIMA:
                self.angulo = self.ROTACAO_MAXIMA
        else:
            if self.angulo > -90: # Começa a rotacionar para baixo quando está caindo
    ...
    def desenhar (self, tela):
        ...
        #não trocar de imagem (bater asas) durante queda
        if self.angulo <= -80:
            self.img = self.IMGS[1]
...
def desenhar_tela(tela, passaros, canos, chao, pontos):
    tela.blit(IMG_DE_FUNDO, (0,0))
    for passaro in passaros:
        passaro.desenhar(tela)
    for cano in canos:
        cano.desenhar(tela)
    
    texto = FONTE_PTS.render(f"PONTUAÇÃO: {pontos}", 1, (255, 255, 255))
...
def main ():
    ...
    while rodando:
        ...
        for i, passaro in enumerate(passaros):
            if (passaro.y + passaro.img.get_height() > chao[0].y or passaro.y < 0):
                passaros.pop(i)

        desenhar_tela(tela, passaros, canos, chao[0], pontos)   
